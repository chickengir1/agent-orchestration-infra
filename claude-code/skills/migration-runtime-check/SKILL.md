...
- 실행 시 `page.locator(selector).count()` 가 정확히 1 이 아니면 step 은 실패로 기록 (`selector-not-found` / `selector-ambiguous`) 하고 같은 action 의 남은 step 은 `skipped`. 다음 action 으로 진행.
- action 실패는 diff 가 아니라 **evidence**. pass/fail 판정 X.
- 각 step 직후 `snapshotAfterEachStep` (default true) 이면 view/actions/meta + screenshot 저장.
