      ...
      <contextId>/
        pages/<scenarioId>/
          ...
          flows/<flowId>/step-<N>/
            ...
            page.png
    <candidateBranch>/          # slug(candidate.branch), e.g. migration-candidate
...
이 스킬은 state machine 으로만 진행한다. 현재 state 의 exit artifact 가 없으면 다음 state 로 가지 않는다. 다음 state 로 넘어간 뒤 이전 질문으로 되돌아가지 않는다. 단, 생성 artifact 가 schema/계약 위반이면 즉시 해당 state 에서 보정하고 다시 exit 한다.

코드 레벨 계약은 두 층이다.

- `check-plan.schema.json`: public JSON shape 문서.
- `contract.py`: 런타임 semantic validator. `plan_helper.py`, `capture.py`, `compare.py` 는 같은 validator 를 사용해야 한다.

`contract.py` 가 막아야 하는 것:

- 중복 context/scenario/flow id
- scenario 가 존재하지 않는 context 를 참조
- unresolved `:param` 이 남은 concrete path
- `flows[]` 누락 또는 빈 배열인데 `no flows selected` 기록 없음
- generated/framework selector
- mutating flow 로 보이는 safe-ui-flow
- token 원문이나 알 수 없는 top-level key 같은 계약 밖 데이터

### S0. Runtime Ready

- input: repo root
- action: Python/venv/Playwright runtime 만 확인
- exit artifact: runtime 사용 가능
- forbidden: app 추정, dev server ping, auth 확인
- next: S1

### S1. Build Unit Selected

- input: repo root
- action: build unit 후보를 조사하고 사용자에게 app/baseline/candidate 만 묻는다
- exit artifact:
  - `targetApp`
  - `baseline.branch`
  - `candidate.branch`
- forbidden: 검수 범위 질문, metadata 질문, discover 실행
- next: S2

### S2. User Info Bound

- input: `targetApp`
- action: 선택지 기반 UX 로 유저 정보 방식을 결정한다
- exit artifact:
  - `auth.storageState` 또는 비로그인 명시
  - `auth.actor`
  - `auth.role` 또는 opaque 실행 주체 label
- forbidden: token 원문 저장, token 원문을 plan/report/stamp/capture 에 기록
- next: S3

### S3. Domain JSON Bound

- input: 사용자의 자유 입력 도메인 지식
- action: 입력 문장에 실제로 존재하는 개념만 JSON 으로 구조화한다
- exit artifact:
  - `contexts[]`
  - `contexts[].vars`
  - `contexts[].labels`
  - `contexts[].metadata`
  - `knownUnstable`
- forbidden: 스킬이 특정 도메인 템플릿을 강요, 도메인 용어 의미 추론
- next: S4

### S4. Flow Intent Bound

- input: 사용자의 자유 입력 업무 흐름/검증 관심사
- action: 유저가 아는 수준의 흐름만 받는다. 페이지 내부 컨트롤/selector 를 요구하지 않는다
- exit artifact:
  - flow intent 후보 목록 또는 명시적 no-flow intent
- forbidden: 유저에게 selector 지식 요구, mutating flow 자동 실행 약속
- next: S5

### S5. Discovery Complete

- input: `targetApp`, auth 상태, base URL
- action: route 구조를 1회 discover 한다
- exit artifact: `<repo-root>/.claude/migration-runtime-check/discover-<app>.json`
- forbidden: discover 결과를 테스트 요구사항으로 간주, branch 별 반복 discover
- next: S6

### S6. Check Plan Written

- input: discover artifact + S2/S3/S4 artifacts
- action: `check-plan-<app>.json` 을 작성한다
- exit artifact:
  - `baseline`
  - `candidate`
  - `auth`
  - `contexts[]`
  - `scenarios[]`
  - scenario 별 `flows[]` 또는 `no flows selected`
- forbidden: silent flows=0, unsafe selector flow, mutating flow, check-plan 없는 capture
- next: S7

### S7. Baseline Captured

- input: check-plan, 사람이 baseline branch checkout/dev server ready
- action: `capture.py --side A` 실행. initial pass 후 retry 후보만 재실행하고 scenario 폴더를 덮어쓴다
- exit artifact:
  - `<run>/<baselineBranch>/stamp.json` with `side=A`
  - `<run>/<baselineBranch>/retry-candidates.json`
  - `<run>/<baselineBranch>/retry-history.json`
  - scenario capture folders
- forbidden: candidate capture 시작, A retry 후보 미처리 방치
- next: S8

### S8. Candidate Captured

- input: 같은 check-plan, 사람이 candidate branch checkout/dev server ready
- action: `capture.py --side B` 실행. initial pass 후 retry 후보만 재실행하고 scenario 폴더를 덮어쓴다
- exit artifact:
  - `<run>/<candidateBranch>/stamp.json` with `side=B`
  - `<run>/<candidateBranch>/retry-candidates.json`
  - `<run>/<candidateBranch>/retry-history.json`
  - scenario capture folders
- forbidden: check-plan 변경, discover 재실행, 다른 viewport/auth/path 사용
- next: S9

### S9. Scenario Matrix Reported

- input: same run directory containing A/B side artifacts
- action: `compare.py` 실행
- exit artifact: `<run>/report.md`
- report contract:
  - `Meta -> Scenario (A/B) -> Signal -> Investigation` 순서
  - 신호 없는 scenario 생략
  - invalid/noise/transient/codebase search 를 scenario matrix 안에 기록
  - Engineering Retrospective 생성 금지
- forbidden: pass/fail 판정, A/B directory name 추정, 같음 나열
- terminal state
...
check-plan 은 capture 계약이다. Claude 는 초안을 만든 뒤 장황한 승인 핑퐁으로 멈추지 않는다. 명백한 모순이나 위험한 state-changing flow 가 없으면 저장하고 다음 단계로 간다.
...
- compare 는 각 branch 디렉터리의 `stamp.json.side` 로 baseline(A) / candidate(B)를 식별한다. 디렉터리 이름을
  `A` / `B` 로 가정하지 않는다.
- main diff 가 없고 노이즈만 잡힌 scenario 도 `## Scenario Matrix` 에 쓰되 `Signal` 에 noise 로 표시한다. Summary 에 `noise-only scenarios` 카운트도 추가.
...
### 11. scenario matrix report.md 생성

같은 점은 쓰지 않는다. 신호 없는 scenario 는 통째 생략한다. report 는 `Meta -> Scenario (A/B) -> Signal -> Investigation` matrix 로만 쓴다. Engineering Retrospective 는 생성하지 않는다.
...
```json
{
  ...
  "candidate": { "branch": "candidate", "baseUrl": "http://localhost:4200" },
  "auth": { "storageState": ".auth/sample-app.json", "actor": "<actor-id>", "role": "<opaque-role>" },
...
- v1 비목표: navigation flow, state-changing flow, destructive flow, external side-effect flow, flow 후보 자동 생성, pixel diff.
...
scenario 의 `expectedFinalPath` 와 capture 의 `finalPath` 가 다르면 **deep diff 를 건너뛰고** report 의 Scenario Matrix 에 invalid signal 로 기록한다. 양쪽 사이드 중 하나라도 expectedFinalPath 에 도달하지 못하면 그 scenario 는 invalid 처리.
...
#### Signal
- invalid: reasonKind=...
- page: finalUrl/title/components/classes/headings/texts/whiteScreen 차이
- action: action surface 차이
- runtime: console/pageerror/requestfailed 차이
- flow: user-flow step 차이
- noise: knownUnstable 로 분리된 후보
- transient: initial snapshot 에만 있고 settled snapshot 에서 사라진 차이
...
- codebase search 후보:
  ...
  - `<source-path>:<line>` ...
- next action:
  - retry / expectedFinalPath 보정 / 코드 조사 / 무시 후보 중 하나

신호 없는 scenario 는 Scenario Matrix 에 쓰지 않는다. Engineering Retrospective 섹션은 생성하지 않는다.
```
...
- state-changing / destructive / external side-effect flow
