...
<repo-root>/.claude/migration-runtime-check/
  discover-<app>.json
  run-<n>/A/<pageId>/{capture.json,page.png}
  run-<n>/A/stamp.json
  run-<n>/B/...
  run-<n>/report.md
```
...
# → run-1/diff.json, run-1/report.md
```

- 입력: `run-<n>/A/<pageId>/capture.json` + `run-<n>/B/<pageId>/capture.json`
- pageId 로 join, A-only / B-only / both 3분류
- per-page 신호 (자동 판정 X, 신호 나열만):
  - `view.title`, `finalUrl` 동등 비교
  - `view.components` 맵 added/removed/countΔ
  - `view.classes` 맵 added/removed/countΔ (CSS load 실패·scoping 회귀 시 변화)
  - `actions` `(role, name, locus)` set diff
  - `console` type ∈ {error, warning} 의 text 첫 줄 set diff (B 신규만)
  - `pageerror` B 신규
  - `requestfailed` `(host+path, failure)` set diff — query string 은 GA nonce 노이즈라 제외
  - white screen 플래그: `len(actions)==0`
- screenshot 은 비교 대상 X (픽셀 비결정성). `page.png` 는 캡처 시점 증거로만 보존. 회귀 판단은 DOM 레벨 결정적 신호 (components/classes/actions/console/pageerror/requestfailed) 로 수행
