...
```bash
cat <<'EOF' | python3 ~/.claude/skills/test-matrix/scripts/generate.py init -
{
  "name": "project_name",
...
```bash
python3 ~/.claude/skills/test-matrix/scripts/generate.py check {repo_root}/.claude/tests/{topic}/
```
